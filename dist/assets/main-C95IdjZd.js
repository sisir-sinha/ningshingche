import{c as T}from"./router-DkUXv8lC.js";function P(){return`
  <!-- NAVBAR -->
  ...
  <section class="max-w-[1120px] mx-auto px-4 sm:px-6 py-8">
  ...
  </section>`}const R=document.getElementById("site");T([{path:"/",view:P,title:"Mekholi — দোকানের হিসাব, এখন আরও সহজ"},{path:"/pricing",view:L,title:"Mekholi Pricing — BDT via bKash"},{path:"/help",view:A,title:"Mekholi Help"},{path:"/login",view:I,title:"Mekholi — Log in"},{path:"/dashboard",view:O,title:"Mekholi — Dashboard"},{path:"/app",view:()=>(location.href="/app.html","")},{path:"/app.html",view:()=>(location.href="/app.html","")}],R);function f(s,t=2200){const l=document.getElementById("toast");l.textContent=s,l.classList.remove("hidden"),setTimeout(()=>l.classList.add("hidden"),t)}window.toast=f;window.addEventListener("mk:navigate",()=>{M()});function M(){const s=document.getElementById("mobile-menu-btn"),t=document.getElementById("mobile-menu");s&&t&&(s.onclick=()=>t.classList.toggle("hidden"),t.querySelectorAll("a").forEach(a=>a.addEventListener("click",()=>t.classList.add("hidden"))),document.querySelectorAll('a[href^="#"]').forEach(a=>{a.addEventListener("click",p=>{p.preventDefault();const u=a.getAttribute("href").slice(1);document.getElementById(u)?.scrollIntoView({behavior:"smooth",block:"start"}),t.classList.add("hidden")})})),document.querySelectorAll('a[href^="#"]').forEach(a=>{a.closest("#mobile-menu")||a.addEventListener("click",p=>{const u=a.getAttribute("href");u.length>1&&(p.preventDefault(),document.getElementById(u.slice(1))?.scrollIntoView({behavior:"smooth"}))})});const l=document.getElementById("lang-toggle"),n=document.getElementById("lang-label");let o=!0;l?.addEventListener("click",()=>{o=!o,n&&(n.textContent=o?"BN / EN":"EN / BN"),f(o?"বাংলা মোড":"English mode")}),D();const e=document.getElementById("bill-monthly"),d=document.getElementById("bill-yearly"),r=document.getElementById("price-main"),c=document.getElementById("price-sub");function y(){e?.classList.add("bg-white","text-slate-900"),d?.classList.remove("bg-white","text-slate-900"),d?.classList.add("text-white"),r&&(r.textContent="৳499"),c?.classList.add("hidden")}function w(){d?.classList.add("bg-white","text-slate-900"),d?.classList.remove("text-white"),e?.classList.remove("bg-white","text-slate-900"),e?.classList.add("text-white"),r&&(r.textContent="৳4,999"),c?.classList.remove("hidden")}e?.addEventListener("click",y),d?.addEventListener("click",w);const k=document.getElementById("otp-send"),S=document.getElementById("otp-verify"),E=document.getElementById("login-phone"),B=document.getElementById("otp-code"),b=document.getElementById("login-msg");k?.addEventListener("click",()=>{const a=E?.value.trim()||"";if(!/^01[3-9]\d{8}$/.test(a)){v("Enter valid phone: 01XXXXXXXXX",!0);return}v("OTP sent (demo) — use 123456",!1)}),S?.addEventListener("click",()=>{(B?.value||"")==="123456"?(v("✓ Logged in (demo). Redirecting to /app …",!1),setTimeout(()=>location.href="/app.html",900)):v("Wrong OTP — try 123456 (demo)",!0)});function v(a,p){b&&(b.textContent=a,b.classList.remove("hidden"),b.style.color=p?"#dc2626":"#059669")}const g=document.getElementById("site-header");if(g){const a=()=>{window.scrollY>8?g.classList.add("shadow-sm"):g.classList.remove("shadow-sm")};window.addEventListener("scroll",a,{passive:!0}),a()}}let i=[],x="58";function D(){const s=document.getElementById("demo-add-rice"),t=document.getElementById("demo-add-oil"),l=document.getElementById("demo-clear"),n=document.getElementById("paper-toggle");s&&(s.onclick=()=>h({id:"rice",name:"Miniket Rice 1kg",price:78}),t&&(t.onclick=()=>h({id:"oil",name:"Parachute Oil",price:180})),l&&(l.onclick=()=>{i=[],m()}),n&&(n.onclick=()=>{x=x==="58"?"80":"58",n.textContent=`Paper: ${x}mm`,f(`Paper → ${x}mm (${x==="58"?"32 chars":"48 chars"})`)}),m())}function h(s){const t=i.find(l=>l.id===s.id);t?t.qty+=1:i.push({...s,qty:1}),m()}function m(){const s=document.getElementById("demo-cart"),t=document.getElementById("demo-totals");if(!s||!t)return;if(i.length===0){s.innerHTML='<div class="text-sm text-slate-500 py-8 text-center border-2 border-dashed border-slate-200 rounded-2xl">Cart empty — click “+ Add”</div>',t.classList.add("hidden");return}s.innerHTML=i.map(e=>`
    <div class="flex items-center justify-between bg-white rounded-xl px-3 py-2.5 border border-slate-200">
      <div>
        ...
        <div class="text-xs text-slate-500">${e.qty} × ৳${e.price}</div>
      </div>
      <div class="flex items-center gap-2">
        <button data-dec="${e.id}" class="w-7 h-7 rounded-full border border-slate-200 grid place-items-center hover:bg-slate-50">−</button>
        <span class="w-6 text-center text-sm font-bold">${e.qty}</span>
        <button data-inc="${e.id}" class="w-7 h-7 rounded-full bg-slate-900 text-white grid place-items-center">+</button>
        <span class="ml-2 font-bold text-sm">৳${(e.price*e.qty).toFixed(2)}</span>
      </div>
    </div>
  `).join(""),s.querySelectorAll("[data-inc]").forEach(e=>e.addEventListener("click",()=>{const d=e.dataset.inc;i.find(r=>r.id===d).qty++,m()})),s.querySelectorAll("[data-dec]").forEach(e=>e.addEventListener("click",()=>{const d=e.dataset.dec,r=i.find(c=>c.id===d);r.qty--,r.qty<=0&&(i=i.filter(c=>c.id!==d)),m()}));const l=i.reduce((e,d)=>e+d.price*d.qty,0),n=+(l*.05).toFixed(2),o=+(l+n-17).toFixed(2);document.getElementById("demo-sub").textContent=`৳${l.toFixed(2)}`,document.getElementById("demo-vat").textContent=`৳${n.toFixed(2)}`,document.getElementById("demo-total").textContent=`৳${o.toFixed(2)}`,document.getElementById("demo-pay").textContent=`৳${o.toFixed(2)}`,t.classList.remove("hidden")}
