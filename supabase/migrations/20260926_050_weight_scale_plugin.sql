-- 050 — The Weighing Scale plugin, seeded as a package.
--
-- sell 1.250 kg of lentils: the till reads your scale’s labels, and the shop sees which items its scale cannot ring up.
--
-- The SQL below lives in `supabase/plugins/weight-scale/` as real files, one per
...
values ('weight-scale', 'Weighing Scale', 'optional', '1.0.0', '^1.0.0',
        'A grocery that cannot weigh cannot operate: this plugin teaches the till to read the shop’s own scale labels, and reports what left the shop by weight and what the scale cannot sell at all.',
        '{}', '{}')
...
values
  ('weight-scale', 'weight-scale.view', 'See weighed sales', 'inventory',
   'See what left the shop by weight, and which items this shop’s scale cannot ring up.'),
  ('weight-scale', 'weight-scale.manage', 'Describe the scale’s labels', 'inventory',
...
begin
  -- ── 001_functions.sql ─────────────────────────────────────────────────
  v_file := '001_functions.sql';
  v_ordinal := 1;
  v_sql := $plg_1$-- Weighing scale — the numbers, in SQL.
--
...
$plg_1$;
  ...
  values ('weight-scale', v_file, '1.0.0', v_ordinal, md5(v_sql), v_sql)
...
declare
  ...
  v_file text;
begin
  foreach v_file in array array['001_functions.sql']
  loop
    if not exists (
      select 1 from public.plugin_package_migrations m
       where m.plugin_key = 'weight-scale' and m.filename = v_file
    ) then
  ...
  if array_length(v_missing, 1) is not null then
    raise exception 'weight_scale_package_incomplete: %', array_to_string(v_missing, ', ')
      using errcode = 'P0001';
  end if;

  if (select count(*) from public.plugin_package_permissions p where p.plugin_key = 'weight-scale') <> 2 then
    raise exception 'weight_scale_package_incomplete: permission rows' using errcode = 'P0001';
  end if;

  if (select count(*) from public.plugin_package_migrations m where m.plugin_key = 'weight-scale') <> 1 then
    raise exception 'weight_scale_package_incomplete: migration rows' using errcode = 'P0001';
  end if;
end
$verify$;
