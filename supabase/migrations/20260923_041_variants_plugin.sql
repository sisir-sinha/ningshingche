-- 041 — The variants plugin, seeded as a package.
--
-- Capability plugin #1 of Phase 7 (docs/10): option types and values, a matrix
-- generator that turns "Colour: Red, Blue" × "Size: S, M, L" into six real
-- variants with one press, and bulk price editing so those six can be priced
-- without opening six dialogs.
--
-- **This plugin adds no new way to store a variant.** `product_variants`,
-- `product_option_types` and `product_option_values` all shipped in 005 — the
-- schema already had the shape, and a variant could already be sold, stocked
-- and priced through the POS. What did not exist was any way to *build* them:
-- a product got exactly one default variant, created with it. So the plugin
-- owns the workflow (which options a product carries, what the combinations
-- are called, what they inherit) and reuses the core's tables for everything
-- else, which is exactly what spec §51 asks for: the universal thing stays
-- universal, the opinionated thing becomes a plugin.
--
-- The SQL below lives in `supabase/plugins/variants/` as real files, one per
-- migration, and is read from disk here. `supabase/schema.sql` is generated
-- from the same files, so the validator replays the identical text rather than
-- a copy that has drifted: a checksum that certifies the wrong thing is worse
-- than no checksum. (Earlier plugin packages inlined their SQL; that is why
-- this one reads it — see docs/09 #12.)

-- ── The package ──────────────────────────────────────────────────────────
insert into public.plugin_packages
      (plugin_key, name, category, version, core_api_version, description,
       dependencies, conflicts)
values ('variants', 'Variants', 'optional', '1.0.0', '^1.0.0',
        'Products with options — colour, size, material — and one press to build every combination.',
        '{}', '{}')
...
values
  ('variants', 'variants.view', 'See product variants',
   'products', 'See a product''s options and the combinations built from them.'),
  ('variants', 'variants.manage', 'Manage product variants',
   'products', 'Choose which options a product has, generate combinations, edit them in bulk.')
...
-- `ordinal` is the run order. `checksum` is md5 of the file's exact text, and
-- `app.plugin_apply_migrations` compares it against `plugin_migrations` on
-- every later enable, so a file edited after a shop applied it is refused
...
begin
  ...
  v_ordinal := 1;
  v_sql := $plg_1$create table if not exists public.plg_variants_product_axes (
...
$plg_1$;
  ...
  values ('variants', v_file, '1.0.0', v_ordinal, md5(v_sql), v_sql)
  ...
  v_ordinal := 2;
  v_sql := $plg_2$create or replace function app.variants_config(p_organization_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  loop
    if not exists (
      select 1 from public.plugin_package_migrations m
       where m.plugin_key = 'variants' and m.filename = v_file
    ) then
  ...
  if array_length(v_missing, 1) is not null then
    raise exception 'variants_package_incomplete: %', array_to_string(v_missing, ', ')
      using errcode = 'P0001';
  end if;

  if (select count(*) from public.plugin_package_permissions p where p.plugin_key = 'variants') <> 2 then
    raise exception 'variants_package_incomplete: permission rows' using errcode = 'P0001';
  end if;
end
$verify$;
