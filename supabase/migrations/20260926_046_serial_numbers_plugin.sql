-- 046 — The Serial Numbers plugin, seeded as a package.
--
-- Capability plugin #3 of Phase 7 (docs/10): a shop that sells things with a
-- number of their own — a handset's IMEI, an engine number, a bicycle frame, a
-- jewellery certificate — tracks which physical unit went out, on which
-- invoice, and when it came back.
--
-- **Nothing here is a new way to sell.** Stock is still the core's ledger, the
-- invoice is still the core's sale, and the receipt is still the core's. What
-- the core cannot know is which of five identical handsets left the shop, so
-- that one fact is all this package owns: a table of units, and the functions
-- that register, attach and release them.
--
-- The SQL below lives in `supabase/plugins/serial-numbers/` as real files, one per
-- migration, and is read from disk here byte for byte — the same arrangement
-- 041 introduced for variants. The validator compares every embedded copy
-- against the file it came from, and against the bundle's manifest, so a
-- version, a permission key or a statement cannot drift between the two halves
-- of a plugin.
--
...
values ('serial-numbers', 'Serial Numbers', 'optional', '1.0.0', '^1.0.0',
...
values
  ('serial-numbers', 'serial-numbers.view', 'See serial numbers', 'inventory',
   'Look up a unit by its number and see what it is, where it is and which sale it left on.'),
  ('serial-numbers', 'serial-numbers.manage', 'Add and capture serial numbers', 'inventory',
...
begin
  ...
  v_ordinal := 1;
  v_sql := $plg_1$-- The serial-number pool.
--
...
$plg_1$;
  ...
  values ('serial-numbers', v_file, '1.0.0', v_ordinal, md5(v_sql), v_sql)
  ...
  v_ordinal := 2;
  v_sql := $plg_2$-- Serial Numbers — behaviour, in SQL.
--
...
begin
  ...
  loop
    if not exists (
      select 1 from public.plugin_package_migrations m
       where m.plugin_key = 'serial-numbers' and m.filename = v_file
    ) then
  ...
  if array_length(v_missing, 1) is not null then
    raise exception 'serial_numbers_package_incomplete: %', array_to_string(v_missing, ', ')
      using errcode = 'P0001';
  end if;

  if (select count(*) from public.plugin_package_permissions p where p.plugin_key = 'serial-numbers') <> 2 then
    raise exception 'serial_numbers_package_incomplete: permission rows' using errcode = 'P0001';
  end if;

  if (select count(*) from public.plugin_package_migrations m where m.plugin_key = 'serial-numbers') <> 2 then
    raise exception 'serial_numbers_package_incomplete: migration rows' using errcode = 'P0001';
  end if;
end
$verify$;
