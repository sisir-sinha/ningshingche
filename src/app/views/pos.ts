  ...
  <div class="min-h-screen bg-slate-50 flex flex-col">
    <header class="sticky top-0 z-30 bg-white border-b border-slate-200">
      <div class="max-w-[1280px] mx-auto px-3 sm:px-4 h-[56px] flex items-center gap-2">
        ...
        <div class="font-bold text-sm sm:text-base">POS Billing</div>
        <span class="hidden sm:inline-flex text-[11px] font-bold tracking-widest bg-emerald-50 border border-emerald-200 text-emerald-700 px-2 py-1 rounded-full">Lite • Offline</span>
        <div class="ml-auto flex items-center gap-2">
          <span id="outbox-badge" class="hidden text-xs font-bold bg-amber-500 text-white px-2.5 py-1 rounded-full">Offline: 0 pending</span>
          <span id="supabase-mini" class="hidden sm:inline-flex text-xs font-semibold text-slate-500"></span>
          <button id="pos-clear" class="text-xs font-bold border border-slate-200 px-3 py-1.5 rounded-full hover:bg-slate-50">Clear</button>
        </div>
      </div>
    </header>

    <div class="flex-1 max-w-[1280px] mx-auto w-full px-3 sm:px-4 py-3 sm:py-4 grid lg:grid-cols-[1.1fr_0.9fr] gap-4 items-start">
      <!-- LEFT: Products -->
      <div class="bg-white border border-slate-200 rounded-[20px] p-3 sm:p-4">
        <div class="flex flex-col sm:flex-row gap-2">
          <div class="flex-1 relative">
            ...
            <input id="pos-search" placeholder="Search name / barcode / SKU — or scan" class="w-full pl-9 pr-9 py-2.5 rounded-full border border-slate-200 outline-none text-sm focus:border-slate-900 focus:ring-2 focus:ring-slate-900/10" autocomplete="off" />
            <button id="pos-scan" class="absolute right-1 top-1 bottom-1 w-8 rounded-full bg-slate-900 text-white grid place-items-center"><span class="material-symbols-rounded text-[18px]">barcode_scanner</span></button>
          </div>
          <div class="flex items-center gap-2">
            <select id="pos-unit-filter" class="px-3 py-2.5 rounded-full border border-slate-200 text-sm bg-white">
              <option value="">All units</option>
              <option value="pcs">pcs</option><option value="kg">kg</option><option value="ltr">ltr</option>
            </select>
            <span id="pos-count" class="text-xs font-bold text-slate-500 whitespace-nowrap">0 products</span>
          </div>
        </div>

        <div id="pos-products" class="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 max-h-[52vh] lg:max-h-[68vh] overflow-auto pr-1">
          <!-- products injected -->
        </div>
        <div class="mt-3 flex items-center justify-between text-[11px] text-slate-500">
          <span>Tip: USB/Bluetooth scanner in HID mode auto-adds. Camera: tap <span class="material-symbols-rounded text-[14px] align-middle">barcode_scanner</span></span>
          <span class="hidden sm:inline">Stock badges at ≤5</span>
        </div>
      </div>

      <!-- RIGHT: Cart + Customer + Pay -->
      <div class="bg-white border border-slate-200 rounded-[20px] p-3 sm:p-4 flex flex-col lg:sticky lg:top-[72px]">
        ...
        <div class="flex gap-2 items-center">
          <div class="flex-1 relative">
            ...
            <input id="pos-customer" placeholder="Customer phone 01XXXXXXXXX or Walk-in" class="w-full pl-9 pr-3 py-2.5 rounded-full border border-slate-200 outline-none text-sm focus:border-slate-900" inputmode="numeric" />
          ...
          <button id="pos-add-customer" class="shrink-0 w-10 h-10 rounded-full bg-slate-900 text-white grid place-items-center"><span class="material-symbols-rounded">person_add</span></button>
        </div>
        <div id="pos-customer-chip" class="hidden mt-2 inline-flex items-center gap-2 text-xs font-bold bg-emerald-50 border border-emerald-200 text-emerald-700 px-3 py-1.5 rounded-full w-fit"></div>
        ...
        <div class="mt-4">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <button id="pos-hold" class="text-xs font-bold border border-slate-200 px-2.5 py-1 rounded-full hover:bg-slate-50">Hold</button>
              <span id="pos-held-badge" class="hidden text-xs font-bold bg-amber-500 text-white px-2 py-1 rounded-full">Held 0</span>
            </div>
          </div>
          <div id="pos-cart" class="mt-2 space-y-2 min-h-[120px] max-h-[32vh] overflow-auto pr-1">
            <div class="text-sm text-slate-500 py-10 text-center border-2 border-dashed border-slate-200 rounded-2xl">Cart empty — tap product or scan</div>
          </div>
        </div>
        ...
        <div class="mt-3 grid grid-cols-2 gap-2">
          <label class="text-xs font-bold text-slate-600">Discount
            <div class="mt-1 flex">
              <input id="pos-discount" type="number" value="0" min="0" class="flex-1 px-3 py-2 rounded-l-full border border-slate-200 outline-none text-sm" />
              <select id="pos-discount-type" class="px-2 py-2 rounded-r-full border-y border-r border-slate-200 bg-slate-50 text-xs font-bold">
                <option value="amount">৳</option><option value="percent">%</option>
              </select>
            </div>
          </label>
          <label class="text-xs font-bold text-slate-600">VAT profile
            <select id="pos-vat-profile" class="mt-1 w-full px-3 py-2 rounded-full border border-slate-200 bg-white text-sm">
              <option value="0">0% (small shop)</option><option value="5">5%</option><option value="7.5">7.5%</option><option value="10">10%</option><option value="15" selected>15% (NBR standard)</option>
            </select>
          </label>
        </div>
        ...
        <div class="mt-3 bg-slate-50 border border-slate-200 rounded-2xl p-3">
          ...
          <div class="flex justify-between font-black text-base mt-2 pt-2 border-t border-slate-200"><span>TOTAL</span><span id="pos-total">৳0.00</span></div>
          <div id="pos-due-row" class="hidden mt-2 flex justify-between text-sm font-black text-red-600 bg-red-50 border border-red-200 rounded-full px-3 py-1.5"><span>BAKI DUE</span><span id="pos-due">৳0.00</span></div>
        </div>
        ...
        <div class="mt-3">
          ...
          <div id="pos-mfs-row" class="hidden mt-3">
            <label class="text-xs font-bold text-slate-600">TrxID (bKash/Nagad/Rocket/Upay) — required for MFS</label>
            <input id="pos-trxid" placeholder="e.g. 9H7K2L9P (8-12 chars)" class="mt-1 w-full px-3 py-2.5 rounded-full border border-slate-200 outline-none text-sm" maxlength="12" />
            <div class="mt-1 text-[11px] text-slate-500">Personal bKash allowed — manual verify. SSLCommerz auto later.</div>
          </div>
          <div id="pos-paid-row" class="hidden mt-3">
            <label class="text-xs font-bold text-slate-600">Paid now (for Due / Split)</label>
            <input id="pos-paid" type="number" value="0" min="0" placeholder="Paid now ৳" class="mt-1 w-full px-3 py-2.5 rounded-full border border-slate-200 outline-none text-sm" />
          </div>
        </div>
        ...
        <button id="pos-pay" class="mt-4 w-full bg-slate-900 text-white rounded-full py-3.5 font-black text-[15px] flex items-center justify-center gap-2 hover:bg-black disabled:opacity-40 disabled:cursor-not-allowed">
          <span class="material-symbols-rounded">print</span> Pay & Print — <span id="pos-pay-total">৳0.00</span>
        </button>
        <div class="mt-2 text-center text-[11px] text-slate-500">Offline → prints immediately, syncs when online. No hash route.</div>

        <!-- Receipt preview (screen) -->
        <div id="pos-receipt" class="hidden mt-4 bg-white border border-slate-200 rounded-2xl p-3 font-mono text-[11px] leading-4">
          <!-- injected -->
        </div>
      </div>
    </div>
...
export function initPos() {
  ...
  const supaMini = document.getElementById('supabase-mini')!
  ...
  async function refreshOutbox() {
    ...
    if(n>0){ outboxBadge.textContent = `Offline: ${n} pending`; outboxBadge.classList.remove('hidden') } else outboxBadge.classList.add('hidden')
  }
  refreshOutbox()
  setInterval(refreshOutbox, 3000)

  // supabase mini status
  supaMini.textContent = isSupabaseConfigured ? 'Supabase ✓' : 'Demo mode'
  ...
  function filterLocal(q:string){ if(!q) return products; const s=q.toLowerCase(); return products.filter(p=> p.name.toLowerCase().includes(s) || (p.barcode && p.barcode.includes(q)) || (p.sku && p.sku.toLowerCase().includes(s)))}

  function renderProducts(list: Product[]) {
    count.textContent = `${list.length} products`
    if (!list.length) { grid.innerHTML = `<div class="col-span-full text-sm text-slate-500 py-10 text-center border-2 border-dashed border-slate-200 rounded-2xl">No products — add in /dashboard or demo data</div>`; return }
    grid.innerHTML = list.map(p=>`
      <button data-add="${p.id}" class="text-left bg-slate-50 hover:bg-white border border-slate-200 hover:border-slate-900 rounded-2xl p-3 flex flex-col gap-2 transition">
        <div class="flex items-start justify-between gap-2">
          ...
          ${p.stock_quantity<=5?`<span class="shrink-0 text-[10px] font-bold bg-amber-500 text-white px-1.5 py-0.5 rounded-full">Low ${p.stock_quantity}</span>`:`<span class="shrink-0 text-[10px] font-bold bg-white border border-slate-200 px-1.5 py-0.5 rounded-full">${p.stock_quantity} left</span>`}
        </div>
        <div class="flex items-center justify-between mt-auto">
          <span class="font-black text-sm">৳${Number(p.price).toFixed(2)} <span class="text-[11px] font-semibold text-slate-500">/${p.unit}</span></span>
        ...
        ${p.barcode?`<div class="text-[10px] font-mono text-slate-500 truncate">${p.barcode}</div>`:''}
      </button>
    `).join('')
  ...
  function addToCart(id:string){
    ...
    else cart.push({ id: Math.random().toString(36).slice(2), product_id:p.id, name:p.name, unit:p.unit, price:p.price, vat_rate:p.vat_rate, qty: p.is_loose?1:1, cost_price: p.cost_price || undefined })
    // if loose, prompt qty?
    renderCart()
  }
  ...
  function renderCart(){
    if(!cart.length){
      cartEl.innerHTML = `<div class="text-sm text-slate-500 py-10 text-center border-2 border-dashed border-slate-200 rounded-2xl">Cart empty — tap product or scan</div>`
      ...
      cartEl.innerHTML = cart.map(c=>`
        <div class="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl px-3 py-2.5">
          <div class="flex-1 min-w-0">
            ...
            <div class="text-xs text-slate-500">${c.qty} ${c.unit} × ৳${c.price.toFixed(2)} • VAT ${c.vat_rate}%</div>
          </div>
          <div class="flex items-center gap-1 shrink-0">
            <button data-dec="${c.id}" class="w-7 h-7 rounded-full border border-slate-200 bg-white grid place-items-center">−</button>
            <span class="w-10 text-center text-sm font-bold">${c.qty}</span>
            <button data-inc="${c.id}" class="w-7 h-7 rounded-full bg-slate-900 text-white grid place-items-center">+</button>
          </div>
          <div class="w-16 text-right font-bold text-sm">৳${(c.price*c.qty).toFixed(2)}</div>
          <button data-del="${c.id}" class="w-7 h-7 rounded-full bg-white border border-slate-200 grid place-items-center text-slate-500 hover:text-red-600"><span class="material-symbols-rounded text-[16px]">close</span></button>
        </div>
      `).join('')
      cartEl.querySelectorAll('[data-inc]').forEach(b=> b.addEventListener('click', ()=>{ const id=(b as HTMLElement).dataset.inc!; const it=cart.find(x=>x.id===id)!; it.qty+=1; renderCart(); updateTotals()}))
      cartEl.querySelectorAll('[data-dec]').forEach(b=> b.addEventListener('click', ()=>{ const id=(b as HTMLElement).dataset.dec!; const it=cart.find(x=>x.id===id)!; it.qty-= (it.unit==='kg'||it.unit==='ltr'?0.5:1); if(it.qty<=0) cart=cart.filter(x=>x.id!==id); renderCart(); updateTotals()}))
      cartEl.querySelectorAll('[data-del]').forEach(b=> b.addEventListener('click', ()=>{ const id=(b as HTMLElement).dataset.del!; cart=cart.filter(x=>x.id!==id); renderCart(); updateTotals()}))
  ...
  function updateTotals(){
    ...
    // due
    const isDue = selectedPay==='due' || selectedPay==='split'
    if(isDue){
      ...
      if(selectedPay!=='split') paidRow.classList.add('hidden')
    }
    // mfs
    if(['bkash','nagad','rocket','upay','bangla_qr'].includes(selectedPay)) mfsRow.classList.remove('hidden')
    else mfsRow.classList.add('hidden')
  }
  ...
  window.addEventListener('keydown', (e)=>{
    ...
    if(e.key==='Enter' && buffer.length>=3){
      ...
      if(p){ e.preventDefault(); addToCart(p.id); (window as any).toast?.('Scanned: '+p.name) }
      return
    }
    if(e.key.length===1) buffer+=e.key
  })

  // customer search (Supabase or local)
  async function searchCustomers(q:string){
    if(!q || q.length<2){ customerList.classList.add('hidden'); return }
    let list: any[] = []
    if(isSupabaseConfigured){
      try{
        const { data } = await supabase.from('customers').select('id,name,phone,due_balance').ilike('phone', `%${q}%`).limit(5)
        if(data) list = data
      }catch{}
    }
    if(!list.length){
      // demo fallback
      const demo = [{id:null, name:'Rahim', phone:'01712345678', due_balance:1170}, {id:null, name:'Karim', phone:'01787654321', due_balance:0}]
      list = demo.filter(d=> d.phone.includes(q) || d.name.toLowerCase().includes(q.toLowerCase()))
    }
    if(!list.length){ customerList.classList.add('hidden'); return }
    customerList.innerHTML = list.map(c=> `<button data-cust='${JSON.stringify(c).replace(/'/g,"&apos;")}' class="w-full text-left px-3 py-2.5 hover:bg-slate-50 flex items-center justify-between"><div><div class="text-sm font-bold">${c.name}</div><div class="text-xs text-slate-500">${c.phone}</div></div><div class="text-xs font-bold ${c.due_balance>0?'text-red-600':'text-emerald-600'}">Due ৳${c.due_balance}</div></button>`).join('')
  ...
  document.getElementById('pos-add-customer')?.addEventListener('click', ()=>{
    ...
    if(!/^01[3-9]\d{8}$/.test(phone)){ (window as any).toast?.('Enter valid phone 01XXXXXXXXX'); return }
  ...
  holdBtn.addEventListener('click', ()=>{
    ...
    heldCarts.push([...cart])
    localStorage.setItem('pos-held', JSON.stringify(heldCarts))
    cart=[]; renderCart(); updateHeldBadge(); (window as any).toast?.('Held ✓')
  })
  heldBadge.addEventListener('click', ()=>{
    if(!heldCarts.length) return
    const last = heldCarts.pop()!
    localStorage.setItem('pos-held', JSON.stringify(heldCarts))
    cart = last; renderCart(); updateHeldBadge(); (window as any).toast?.('Resumed held')
  })
  ...
  payBtn.addEventListener('click', async ()=>{
    ...
    const vatDefault = parseFloat(vatProfile.value||'0')
    const { subtotal, discountAmount, vat, total } = calcTotals(cart, discountVal, dtype, vatDefault)
    const paidNow = selectedPay==='due' ? (parseFloat(paidInput.value||'0')||0) : (selectedPay==='split' ? (parseFloat(paidInput.value||'0')||0) : total)
    const due = Math.max(0, total - paidNow)
    if(due>0 && !selectedCustomer){ (window as any).toast?.('Select customer for Baki/Due'); customerInput.focus(); return }
    if(['bkash','nagad','rocket','upay','bangla_qr'].includes(selectedPay) && !trxInput.value.trim()){
      if(!confirm('MFS selected but TrxID empty. Pay without TrxID?')) return
    }
    ...
    // get store_id & cashier_id — try Supabase auth
    ...
    try{
      ...
      if(user){
        ...
        store_id = (prof as any)?.store_id || null
      }
    }catch{}
    ...
    const orderPayload: any = {
      ...
      is_due: due>0,
      due_amount: due,
      mfs_trxid: trxInput.value.trim() || null,
      customer_id: selectedCustomer?.id || null,
      client_uuid,
      bin_snapshot: null,
      note: selectedCustomer ? `Customer: ${selectedCustomer.name} ${selectedCustomer.phone}` : null,
    }

    const itemsPayload = cart.map(c=> ({
      order_id: null as any, // will be set after order insert if needed, but we try upsert with order_id via client flow — for demo we insert separately
      product_id: c.product_id,
      quantity: c.qty,
      unit_price: c.price,
      vat_rate: c.vat_rate,
      vat_amount: Math.round(c.price*c.qty*(c.vat_rate/100)*100)/100,
      unit_snapshot: c.unit,
      client_uuid: Math.random().toString(36).slice(2),
    }))

    // Try Supabase insert if configured and online and store_id present
    let savedOnline = false
    if(isSupabaseConfigured && store_id && navigator.onLine){
      try{
        const { data: orderRow, error: oErr } = await supabase.from('orders').insert(orderPayload).select('id').single()
        if(oErr) throw oErr
        const orderId = (orderRow as any).id
        // insert items with order_id
        const itemsWithOrder = itemsPayload.map(it=> ({ ...it, order_id: orderId }))
        const { error: iErr } = await supabase.from('order_items').insert(itemsWithOrder as any)
        if(iErr) throw iErr
        // if due, create khata
        if(due>0 && selectedCustomer?.id){
          await supabase.from('khata_entries').insert({ store_id, customer_id: selectedCustomer.id, order_id: orderId, type:'dilam', amount: due, note: `Sale ${receipt_number}`, client_uuid: Math.random().toString(36).slice(2) } as any)
        }
        savedOnline = true
      }catch(e:any){
    ...
    if(!savedOnline){
      // offline queue — save to idb outbox
      ...
      if(due>0){
        await enqueue('khata_entries', { store_id: store_id||'demo-store', customer_id: selectedCustomer?.id || null, type:'dilam', amount: due, note: `Sale ${receipt_number} (offline)`, client_uuid: Math.random().toString(36).slice(2) })
      }
      refreshOutbox()
    }
    ...
    receiptEl.innerHTML = `
      <div class="text-center font-black text-[13px]">MEKHOLI — SYLHET</div>
      <div class="text-center text-[10px] text-slate-500">BIN: demo • MUSHAK 6.3 • ${new Date().toLocaleString('en-GB')} • ${receipt_number}</div>
      ...
      <div class="flex justify-between font-black"><span>TOTAL</span><span>৳${total.toFixed(2)}</span></div>
      <div class="flex justify-between text-emerald-600 font-bold"><span>Paid (${selectedPay}${trxInput.value? ' '+trxInput.value:''})</span><span>৳${paidNow.toFixed(2)}</span></div>
      ${due>0?`<div class="flex justify-between text-red-600 font-black bg-red-50 rounded px-1 py-0.5 mt-1"><span>BAKI DUE</span><span>৳${due.toFixed(2)}</span></div>`:''}
      <div class="text-center mt-2 text-[10px] text-slate-400">Dhonnobad! Abar ashben. ${savedOnline?'✓ Synced':'◌ Queued offline'}</div>
    `
    // Print via browser print for demo (thermal print via Bluetooth later)
    // window.print() would print whole page — we just show preview

    ;(window as any).toast?.(savedOnline ? `Paid ৳${total.toFixed(2)} ✓ Synced` : `Paid ৳${total.toFixed(2)} ◌ Queued (${due>0?'Due ৳'+due.toFixed(2):'Paid'})`)

    // Offer Tagada SMS if due
    if(due>0 && selectedCustomer){
      if(confirm(`Baki ৳${due.toFixed(2)} — Send Tagada SMS to ${selectedCustomer.phone}?`)){
        const msg = `Assalamu Alaikum, apnar baki ৳${due.toFixed(2)} — ${receipt_number} — Mekholi Store. Dhonnobad!`
        window.location.href = `sms:${selectedCustomer.phone}?&body=${encodeURIComponent(msg)}`
      }
    }

    cart=[]; renderCart(); payBtn.disabled=false; payBtn.innerHTML = `<span class="material-symbols-rounded">print</span> Pay & Print — <span id="pos-pay-total">৳0.00</span>`
    // re-bind payTotal after reset
    ;(document.getElementById('pos-pay-total') as HTMLElement).textContent = `৳0.00`
  })
