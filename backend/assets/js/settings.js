(function (NC) {
  ...
  async function checkSchema() {
    ...
    try {
      ...
      output.innerHTML = `<div class="diagnostic-grid">${probe.results.map((item) => `<div class="diagnostic-item"><span class="diagnostic-dot ${item.ok ? 'is-ok' : 'is-error'}"></span><code>${escapeHTML(item.table)}</code><span title="${escapeHTML(item.error?.message || '')}">${item.ok ? 'Ready' : item.error?.isAccessControlMissing ? 'Migration 004 needed' : item.error?.isSchemaMissing ? 'Missing' : item.error?.isSchemaMismatch ? 'Columns missing' : `Error ${item.error?.status || ''}`}</span></div>`).join('')}</div>${(() => { const issue = NC.api.schemaBanner(probe); return issue ? NC.components.notice(issue.message, 'warning') : NC.components.notice('All required Supabase tables, media columns, and access-control functions are ready.', 'success'); })()}`;
