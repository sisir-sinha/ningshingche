...
test('the public profile shows the total and what it is made of', () => {
  ...
  assert.match(breakdown, /add\("প্রবন্ধ \$\{toBengaliNumeral\(profile\.articleViews\)\}"\)/);
  assert.match(breakdown, /add\("গান \$\{toBengaliNumeral\(profile\.musicViews\)\}"\)/);
  assert.match(breakdown, /add\("আলোচনা \$\{toBengaliNumeral\(profile\.forumViews\)\}"\)/);
  assert.match(breakdown, /add\("পাঠক \$\{toBengaliNumeral\(profile\.visitors\)\}"\)/);
