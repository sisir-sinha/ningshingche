...
test('the paperclip takes pictures and PDFs, five at most', async (t) => {
  ...
  await t.test('the picker offers pictures and PDFs, and the app checks again', () => {
    ...
    // Re-anchored for the language batch: the count is a `{1}` slot filled by an
    // argument now, so the whole sentence is one key a translator can move.
    assert.match(thread, /একটি উত্তরে সর্বোচ্চ \{1\}টি ফাইল যুক্ত করা যাবে।",\s*toBengaliNumeral\(ForumAttachmentUploader\.MAX_FILES\)/,
